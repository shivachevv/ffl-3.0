<template>
  <div>
    ...
    <div v-if="ready">
      ...
      <table>
        <tbody>
          <tr v-for="(team, i) in result" :key="i">
            <td>{{ team.team }}</td>
            <td>{{ team.league }}</td>
            <td>{{team.tactics}}
              <!-- <span v-for="(tactic, i) in team.tactics" :key="i">
                {{ tactic }}
              </span> -->
            </td>
          </tr>
...
export default {
  ...
  updated() {
    const team = document.querySelector('#verein_head div div div div div h1 span').innerText
    const league = document.querySelector('.hauptpunkt a').innerText
    const tactics = document.querySelectorAll('.aufstellung-vereinsseite')[0].innerText
