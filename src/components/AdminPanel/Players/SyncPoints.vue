  ...
  data () {
    return {
      ...
      form: {
        ...
        to: '',
        format: 'json',
        target: 'https://ffl-project-2cce4-default-rtdb.europe-west1.firebasedatabase.app/pointsSync/points.json'
      },
  ...
  methods: {
    ...
    async uploadHandler () {
      ...
      fetch(
        ...
        // `${toolUrl}?league=${league}&from=${from}&till=${to}&format=${format}&target=${target}`,
        {
          method: 'GET',
          mode: 'cors',
          headers: {
    ...
    async syncPointsHandler () {
      ...
      if (this.points !== 'empty') {
        ...
        await this.fetchUpdatedPlayersObject(updatedPlayers)

        // UPDATE LAST_UPDATE_DATE FOR THE CACHE
        setLastUpdateDB()

        await this.deleteStandings()
        await this.fetchUpdatedStandingsObject1(standings)
